...
<?php include('session.php'); ?>
    <body>
        ...
        <div class="container-fluid">
            <div class="row-fluid">
				<?php include('sidebar/admin_notification_sidebar.php'); ?>
                <div class="span9" id="content">
                     <div class="row-fluid">
  					...
  					<form id="domainTable" action="read_admin.php" method="post">
						<?php  if($not_read == '0'){
								}else{  ?>
					...
					where admin_class.admin_id = '$session_id'  order by  admin_notification.date_of_notification DESC
					")or die(mysqli_error());
					$count = mysqli_num_rows($query);
					while($row = mysqli_fetch_array($query)){
					$assignment_id = $row['assignment_id'];
					$get_id = $row['admin_class_id'];
					$id = $row['admin_notification_id'];
					
					
					$query_yes_read = mysqli_query($conn,"select * from notification_read_admin where notification_id = '$id' and admin_id = '$session_id'")or die(mysqli_error());
